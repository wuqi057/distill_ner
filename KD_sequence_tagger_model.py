...
from data_KD import Dictionary, Sentence, Label #NOTE
...
class SequenceTagger(flair.nn.Model):
    ...
    def _init_model_with_state_dict(state):
        ...
        model = SequenceTagger(
            ...
            use_soft_labels=state['use_soft_labels'],
            use_logits=state['use_logits'],
        )
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        if self.use_rnn:
            ...
            if self.train_initial_hidden_state:
                ...
                rnn_output, hidden = self.rnn(sentence_tensor, initial_hidden_state) #NOTE
            else:
                # rnn_output, hidden = self.rnn(packed)
                rnn_output, hidden = self.rnn(sentence_tensor)#NOTE

            # sentence_tensor, output_lengths = torch.nn.utils.rnn.pad_packed_sequence(
            #     rnn_output, batch_first=True
            # )

            sentence_tensor = rnn_output.to(flair.device)#NOTE
        ...
        if self.use_soft_labels and not self.use_logits: #NOTE: change for KD, logits
    ...
    def _calculate_loss( #NOTE
        ...
        if self.use_soft_labels:
            tag_logprob_dist_list: List = []
            for s_id, sentence in enumerate(sentences):
                # get the tags in this sentence
                logits = sentence.logits.numpy()
                logits = torch.tensor(logits,device=flair.device)
                tag_logprob_dist: List[List[float]] = F.log_softmax(logits,dim=1) 
                #NOTE: make sure all logits has dimension (nr(words in sentence),20)

                # tag_logprob_dist = torch.tensor(tag_logprob_dist, device=flair.device)
                tag_logprob_dist_list.append(tag_logprob_dist)
        ...
        if self.use_logits:
            ...
            for sentence in sentences:
                # get the tags in this sentence
                logits = sentence.logits.numpy()
                logits:List[List[float]] = torch.tensor(logits,device=flair.device)
                # add tags as tensor
        ...
        if self.use_crf:
            ...
            self.used_lossFunction = 'CRF scores'

            return score.mean()
            
        elif self.use_soft_labels and not self.use_logits: 
            score = 0
            for sentence_feats, sentence_tags_dist, sentence_length in zip(
                    features,tag_logprob_dist_list,lengths,
            ):
                sentence_feats = sentence_feats[:sentence_length]
                score += F.kl_div(
                    sentence_feats, sentence_tags_dist,log_target=True,reduction='batchmean',
                )#NOTE:Q and P are both in log space, however, log_target=True only specify if P is in logspace(Q is candidate and P is target)
                 #NOTE: here Q and P are both batches, in the sence that it is catted matrices for the prob. dist for a sentence
                 # dimension here is: (len(sent),20) where 20 is size of tag dictionary. 

            score /= len(features)
            self.used_lossFunction = 'kl_div'
            return score

        elif self.use_soft_labels and self.use_logits:
            score = 0
            for sentence_feats, sentence_logits,sentence_length in zip(
                    features, logits_list,lengths
            ):
                sentence_feats = sentence_feats[:sentence_length]
                score += F.mse_loss(
                    sentence_feats, sentence_logits
                )            
            score /=len(features)
            self.used_lossFunction = 'mse_loss'
            return score
            ...
            score /= len(features)
            self.used_lossFunction = 'cross_entropy'
            return score
    ...
    def _obtain_labels(
        ...
        if self.use_crf:
            ...
            if self.use_soft_labels and not self.use_logits:
                softmax_batch=torch.exp(feature).cpu()#NOTE: in this case feature is log_sofmax of logits
            else:
